...
df = pd.read_excel("/Users/valentindans/Downloads/pricing_data.xlsx", header=None, index_col=0)
df = df.fillna(0)
df = df.T
df1 = pd.melt(frame=df, id_vars=["flat", "type"], var_name="price_date")
df2 = df1.pivot(index=["price_date", "flat"], values="value", columns=["type"])
df2 = df2.reset_index()

# Rename correctly:
df2 = df2.rename(columns={"flat": "object", "Min.": "min_nights", "Price": "price"})
