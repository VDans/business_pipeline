...
date_from = pd.Timestamp(day=1, month=9, year=2023)
date_to = pd.Timestamp(day=30, month=9, year=2023)
...
for f in flats:
    ...
    gv_table.to_csv(path_or_buf=f"{OUTPUT_PATH}/{date_to.strftime('%Y%m')}_{f}.csv",
                    sep=";",
                    index=False)
